...
* **TruffleHog is AGPL-3.0, and its integration is a RATIFIED, documented exception**
  to the earlier ecosystem decision that rejected TruffleHog on license grounds
  (ratified by the maintainer, 2026-07-12). The exception is scoped to exactly the
  boundary the code already enforces: the engine is optional and **off by default**;
  TruffleHog is an operator-installed external executable invoked **subprocess-only**
  — it is never imported, never linked, never bundled, never shipped, and never a pip
  dependency; and `--no-verification` is forced unless the operator explicitly sets
  `allow_network_verification`. BrainConnect neither distributes nor modifies
  TruffleHog, so running it at arm's length does not pull AGPL obligations into this
  codebase. Engine selection remains by capability, so Gitleaks (MIT) or
  detect-secrets (Apache-2.0) substitute freely where an operator prefers them.
