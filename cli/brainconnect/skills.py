    ...
    for other in repo.q(
        ...
        if max(jc, jt) >= REDUNDANCY_THRESHOLD:
            out.append(f"overlaps {other['name']!r} (claims {jc:.2f}, text {jt:.2f}) "
                       f"— consider `brainconnect skill merge` instead of a duplicate")
    return out
...
def archive(repo: Repo, name: str) -> None:
    ...
    if row["installed"]:
        raise SkillError(f"error: {name!r} is installed globally; "
                         f"run `brainconnect skill uninstall {name}` first")
...
def redundant_pairs(repo: Repo) -> list[dict]:
    ...
    cross-skill audit `brainconnect skill suggest`'s create-time dedup can't catch."""
...
def merge(repo: Repo, old: str, into: str) -> None:
    ...
    if o["installed"]:
        raise SkillError(f"error: {old!r} is installed globally; "
                         f"run `brainconnect skill uninstall {old}` first")
...
def _content(repo: Repo, row) -> str:
    ...
    out = [
        ...
        "     `skills` table in the brain DB; run `brainconnect skill render`, never hand-edit. -->",
        "",
        row["body"].rstrip(), "",
        "---", "",
