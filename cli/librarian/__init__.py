...
session. It is a separate console script (`wiki-librarian`) and a separate
process — `wiki` never imports it.

Design:
- Triggered by events, not schedules. Ingest (`wiki add/drop/capture/transcribe`)
  can spawn `wiki-librarian extract --source N` the moment a source lands
  (opt-in via `[librarian] auto_extract`), and `wiki-librarian catch-up`
...
- Provider-agnostic. One thin OpenAI-compatible chat client covers Ollama /
  ...
  in the repo (`wiki lint` still enforces that).
