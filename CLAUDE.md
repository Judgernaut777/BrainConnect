...
- Knowledge flows one way: raw sources → SQLite DB (truth) → generated `wiki/`.
  ...
  hand-edit pages under `wiki/` — change the DB and run `brainconnect render`.
- The live DB is at an absolute path (`config.toml` → `paths.db`,
  default `~/.wiki-brain/wiki.db`), NOT in the working tree.
- The `brainconnect` CLI contains zero model calls (billing + determinism boundary).
...
- Every mutating `brainconnect` command refreshes `db/dump.sql` and appends to `log.md`.
